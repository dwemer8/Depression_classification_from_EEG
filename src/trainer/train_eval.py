...
from .helpers import vizualize, eval_ml_model, eval_dl_model
    ...
    try:
        ...
        for step, imgs in enumerate(dataloader):
            if model_type == "unsupervised":
                ...
                y = torch.tensor(imgs[1]).long()

                y_hat = model(x.to(device))
                loss_mean = criterion(y_hat, y.to(device))
            ...
            if model_type == "unsupervised":
                ...
                #y_proba = nn.functional.softmax(y_pred, dim=1)[:, 1]

                logger.update({
                    'loss': loss_mean.cpu(),
                    'accuracy': (y_pred.cpu() == y.cpu()).sum()/len(y.cpu()),
                    'batch_size': len(y.cpu())
                })
